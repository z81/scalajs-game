...
class WarFog(gridSize: Int, impassableBlocks: Seq[ImageRegion]) {
  ...
  def render(ctx: CanvasRenderingContext2D, player: Player, map: Seq[Seq[ImageRegion]]): Unit = {
    ...
    ctx.rect(0, 0, mapPxWidth, player.y * (gridSize + 1) - maxVisibleRadius)
    ctx.rect(0, player.y * gridSize + maxVisibleRadius, mapPxWidth, mapPxHeight)
    ctx.rect(0, 0, player.x * (gridSize + 1) - maxVisibleRadius, mapPxHeight)
    ctx.rect(player.x * (gridSize + 1) + maxVisibleRadius, 0, mapPxWidth, mapPxHeight)
    // ctx.fill()
